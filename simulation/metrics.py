...
def calculate_rewards(state, config):
    ...
    # --- 2. 运营商利润 (协调后) ---
    peak_hours = grid_status_dict.get("peak_hours", []) # Keep using grid_status for peak/valley hours
    ...
    for c in chargers:
        if isinstance(c, dict):
            charger_id = c.get('id', c.get('charger_id')) # Accommodate both possible keys
            if charger_id:
    ...
    waiting_users = sum(1 for c in chargers if len(c.get('queue', [])) > 0) # This counts chargers with queues, not total waiting users
    utilization = occupied_chargers / total_chargers if total_chargers > 0 else 0
    queue_utilization = waiting_users / total_chargers if total_chargers > 0 else 0 # Proportion of chargers that have a queue
    ...
    # 6. Carbon intensity factor
    carbon_intensity_factor = 0.0
    ci_thresholds = grid_friend_cfg.get('carbon_intensity_thresholds', [400,600,800]) # gCO2/kWh
    ...
    if regional_current_state and len(ci_thresholds) == 3 and len(ci_scores) == 4:
        carbon_intensities = [region.get('carbon_intensity', ci_thresholds[1]) for region in regional_current_state.values() if region] # Default to mid if missing
    ...
    history = state.get('history', []) # Re-fetch for clarity, though available
    if len(history) >= time_window_steps: # Use same window as profit
        ...
        if prev_state_for_load_change and 'grid_status' in prev_state_for_load_change:
            ...
            if prev_load is not None and total_load_abs is not None and prev_load > 1e-6: # Avoid division by zero
    ...
    if regional_connections:
        ...
        total_capacity = sum(conn.get('transfer_capacity', 1000) for conn in regional_connections.values()) # Default capacity if not specified
        if total_capacity > 1e-6:
            transfer_ratio = abs(total_transfer) / total_capacity # Use abs for total magnitude
    ...
    else: grid_friendliness = min(1.0, grid_friendliness_raw * pos_adjust) # Clamp positive score at 1.0 after boost
    ...
    # Base rewards are already in results from the call above
    ...
    # Extract time series data for algorithm comparison
    grid_time_series = state.get('grid_status', {}).get('time_series_data_snapshot', {})
    coordinated_load_profile_regional = grid_time_series.get('regional_data', {})
    timestamps = grid_time_series.get('timestamps', [])
    num_steps = len(timestamps)
    ...
    if num_steps > 0 and coordinated_load_profile_regional:
        # Aggregate total load and renewables from regional data
        for region_id, region_data in coordinated_load_profile_regional.items():
            for i in range(num_steps):
    ...
    uncoordinated_load_profile = state.get('uncoordinated_load_profile') # This should ideally be passed in state
    if not uncoordinated_load_profile and coordinated_total_load_profile:
    ...
    uncoordinated_renewable_profile = coordinated_total_renewable_gen_profile # Assuming same renewable profile

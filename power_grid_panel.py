...
# In class PowerGridPanel:
    ...
    def _create_load_monitoring_section(self):
        ...
        # 增加表格高度
        self.load_table.setMinimumHeight(200)
        self.load_table.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Expanding)
    ...
    def update_load_data(self, combined_grid_data):
        ...
        if num_timestamps>0:
            ...
            if ev_load_latest_kw>1e-3:
                # 修复新能源占比计算，确保不超过100%
                renewable_used_for_ev = min(ev_load_latest_kw, renewable_gen_latest_kw)
                share = (renewable_used_for_ev / ev_load_latest_kw) * 100
                share = min(share, 100.0)  # 确保不超过100%
                self.renewable_share_label.setText(f"可再生能源充电占比: {share:.1f} %")

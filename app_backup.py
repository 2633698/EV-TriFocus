...
def run_simulation():
    ...
    try:
        ...
        use_mock_data = data.get('useMockData', True)
        save_result = data.get('saveResult', False)
        
        print(f"使用策略: {strategy}, 天数: {days}, 完整分析: {full_analysis}, 模拟数据: {use_mock_data}")
        ...
        if strategy in config['strategies']:
            ...
            if use_mock_data:
                # 使用模拟数据
                metrics, avg_metrics, charts, analysis_data = generate_mock_simulation(days, full_analysis, strategy, timestamp)
            else:
                try:
                    # 使用真实计算
                    ...
                    error_msg = str(e)
                    print(f"真实计算过程中发生错误: {error_msg}")
                    # 如果真实计算失败，回退到模拟数据
                    simulation_progress = {"percent": 10, "message": f"真实计算失败，回退到模拟数据: {error_msg}"}
                    metrics, avg_metrics, charts, analysis_data = generate_mock_simulation(days, full_analysis, strategy, timestamp)
...
def run_real_simulation(config, strategy, days, full_analysis, timestamp):
    ...
    def progress_callback(current, total, message=None):
        percent = min(int(current / total * 80) + 10, 90)  # 进度从10%到90%
        msg = message if message else f"模拟中: {current}/{total}"
    ...
    # 运行模拟
        ...
        metrics, avg_metrics = system.run_simulation(days=days, progress_callback=progress_callback)
        print("模拟计算完成，生成结果...")
        
        # 生成图表路径
        main_chart_path = f"/results/simulation_result_{timestamp}.png"
        charts = [main_chart_path]
        ...
        if full_analysis:
            ...
            print("开始生成用户行为分析...")
            user_behavior = system.analyze_user_behavior(num_days=days, progress_callback=progress_callback)
            
            simulation_progress = {"percent": 95, "message": "生成电网影响分析..."}
            print("开始生成电网影响分析...")
            grid_impact = system.analyze_grid_impact(num_days=days, progress_callback=progress_callback)
            ...
            # 添加额外图表路径
            charts.extend([
                f"/output/hourly_charging_demand.png",
                f"/output/user_type_distribution.png",
                f"/output/grid_load_comparison.png",
                f"/output/charging_patterns.png",
                f"/output/user_satisfaction_matrix.png"
            ])
...
def generate_mock_simulation(days, full_analysis, strategy, timestamp):
    """生成模拟的模拟数据"""
    global simulation_progress
    ...
    for i in range(total_steps):
        # 更新进度
        percent = min(int(i / total_steps * 90) + 5, 95)
        simulation_progress = {"percent": percent, "message": f"模拟数据生成: {i+1}/{total_steps}"}
        
        # 模拟计算延时
        time.sleep(0.05)
        
        # 生成随机数据
        if i % (total_steps // (4 * days)) == 0:  # 每天6小时记录一次数据
            metrics["user_satisfaction"].append(random.uniform(0.65, 0.95))
            metrics["operator_profit"].append(random.uniform(0.6, 0.9))
            metrics["grid_friendliness"].append(random.uniform(0.7, 0.95))
            metrics["total_reward"].append(random.uniform(0.65, 0.9))
    ...
    # 生成主图表
    simulation_progress = {"percent": 96, "message": "生成模拟结果图表..."}
    main_chart_path = f"/results/simulation_result_{timestamp}.png"
    create_metrics_chart(metrics, strategy, main_chart_path)
    
    # 初始化图表列表
    charts = [main_chart_path]
    
    # 生成用户行为和电网分析图表
    hourly_demand_path = f"/results/hourly_demand_{timestamp}.png"
    user_types_path = f"/results/user_types_{timestamp}.png"
    grid_load_path = f"/results/grid_load_{timestamp}.png"
    
    # 创建小时充电需求图表
    create_hourly_demand_chart(hourly_demand_path)
    charts.append(hourly_demand_path)
    
    # 创建用户类型分布图表
    create_user_types_chart(user_types_path)
    charts.append(user_types_path)
    
    # 创建电网负荷对比图表
    create_grid_load_chart(grid_load_path)
    charts.append(grid_load_path)
    
    # 如果需要完整分析，生成更多数据
    analysis_data = {}
    if full_analysis:
        simulation_progress = {"percent": 98, "message": "生成用户行为和电网分析数据..."}
        
        # 生成用户行为分析数据
        user_behavior = {
            "user_types": {
                "commuters": random.uniform(0.4, 0.6),
                "shoppers": random.uniform(0.2, 0.3),
                "travelers": random.uniform(0.1, 0.2),
                "residents": random.uniform(0.1, 0.2)
            },
            "charging_patterns": {
                "morning": random.uniform(0.2, 0.3),
                "afternoon": random.uniform(0.3, 0.4),
                "evening": random.uniform(0.2, 0.3),
                "night": random.uniform(0.1, 0.2)
            },
            "avg_charging_time": random.uniform(1.5, 3.0),
            "peak_hours": [8, 9, 17, 18, 19],
            "satisfaction_by_type": {
                "commuters": random.uniform(0.7, 0.9),
                "shoppers": random.uniform(0.6, 0.8),
                "travelers": random.uniform(0.5, 0.7),
                "residents": random.uniform(0.7, 0.9)
            }
        }
        ...
        grid_impact = {
            ...
            "load_shifting": random.uniform(0.2, 0.4),
            "renewable_utilization": random.uniform(0.3, 0.6),
            "grid_stability_score": random.uniform(0.7, 0.9),
            "hourly_load": [random.uniform(30, 90) for _ in range(24)],
            "hourly_load_no_scheduling": [random.uniform(40, 100) for _ in range(24)],
            "carbon_emissions_reduction": random.uniform(0.1, 0.4)
        }
